...
% PCA-based alignment of trajectory data, then saves it all out to a CSV.
...
% 
%             %Checks if it's forwards or backwards, sets directionality
%             %boolean appropriately
%             if(strcmp(directionality(1,1), "F"))
%                 directionality = 0*ones(length(directionality),1);
%                 %disp("F found");
%             elseif(strcmp(directionality(1,1), "B"))
%                 directionality = ones(length(directionality),1);
%                 %disp("B found");
%             else
%                 warning('Unexpected data found in directionality.');
%                 disp(directionality(1,1));
%             end
            ...
            %Making v2 with duplicate positions removed
            %Making v3 with post-experiment changes
            folderPath = strcat('../PosPCADataV3/',newDirname, '/');
            ...
            csvname = strcat(folderPath, sbjFileName, '_',num2str(task), '.csv');
